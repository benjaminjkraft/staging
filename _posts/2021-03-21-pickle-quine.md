...
Third, we'll need the `REDUCE`.  `REDUCE` is used to construct classes[^classes] and in a few other cases.  It requires two things to be on the stack: on top, a tuple of arguments, and below it, a callable; it calls `callable(*arguments)`.  This, plus `TUPLE2` lets us call a function with two arguments: we push the function onto the stack, followed by the two arguments, then call `TUPLE2` to make the arguments into a pair, and `REDUCE` to call them.
